...
Dashboard содержит встроенный c4_lib для прямого подключения к К4. Exporter API доступен как отдельный сервис через nginx mTLS.
...
- Экспорт конфигураций всех шлюзов в tar.gz архив
...
Особенности:
- Прямое подключение к К4 через c4_lib (ГОСТ TLS, без отдельного экспортера в демо)
...
4. dev-c4-dashboard        (Django + c4_lib)
```
...
| `REQUEST_TIMEOUT` | `300` | Таймаут чтения (сек) |
