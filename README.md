...
| Журналы ЦУС | Правила МЭ (источники, назначения, сервисы, приложения, счетчик, установка) |
|---|---|
| ![Logs](c4_dash-demo/3.png) | ![Firewall Rules](c4_dash-demo/5.png) |

| Сетевые объекты | Объекты сервисов |
|---|---|
| ![Network Objects](c4_dash-demo/6.png) | ![Service Objects](c4_dash-demo/7.png) |

| Конфигурация (экспорт, синхронизация, импорт) | Обслуживание (индексы, БД логов ЦУС, автоочистка) |
|---|---|
| ![Configuration](c4_dash-demo/8.png) | ![Maintenance](c4_dash-demo/9.png) |
...
Та же архитектура, что и демо. Все сервисы на `network_mode: host`. Код монтируется через volume для live-reload.
...
| **Политики и объекты** | Правила МЭ (источники, назначения, сервисы, приложения, счетчик, установка на узлы), Сетевые объекты, Объекты сервисов, Приложения |
...
- Экспорт конфигураций всех шлюзов в tar.gz архив с выбором формата:
  - **Универсальный JSON** — правила МЭ/NAT с вложенными объектами (источники, назначения, сервисы, приложения, группы), без UUID, готов к анализу и переносу
...
- ГОСТ CA + сертификаты для nginx и dashboard (mTLS)
...
3. dev-c4-dashboard        (Django + c4_lib)
```
